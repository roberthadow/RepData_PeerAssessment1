...
over a two month period in 2012. 

Preliminary observations:  The subject walks most between 8:00 am and 10:00 am. The subject's walking is more concentrated on weekdays than weekends, but the total distance walked is greater on weekends. Weekends are more consistent than weekdays.

Part of the exercise is imputation of missing data values.  The missing data is spectacularly, even suspiciously, uniform. The analysis is not included here. Nevertheless, we impute the missing data according to assumptions described below.
...
activity %>% group_by(date) %>% summarise(stepsum = sum(steps , na.rm = TRUE)) %>%
        with(hist(stepsum, main = "All Days -- Original Data", breaks = 30, # xaxt = "n",
...
activity %>% group_by(interval) %>% 
        ...
        with(plot(interval, meansteps, main = "Average Daily Pattern",
...
**Assumption:**  A missing value is the same as the average value for the corresponding time interval for the duration of the observation period. NA values are randomly spread through the data (inconsistent with this data).  
...
activity %>% 
        ...
        group_by(interval) %>% 
        summarise(meansteps = mean(steps, na.rm = TRUE)) %>%
        with(plot(interval, meansteps, main = "Weekend Average Activity",
                  xlab = "Time of Day (hhmm)", 
                  ylab = "Steps per five minutes (ave)"))

# Calculate summary statistics weekend activity
activity %>% 
        filter(weekdays(date) %in% c("Sunday", "Saturday")) %>%
        group_by(date) %>% summarise(stepsum = sum(steps , na.rm = TRUE)) %>%
        summarise_each(funs(mean(.,na.rm = TRUE), 
                            median(.,na.rm = TRUE),
                            sd(.,na.rm = TRUE)), stepsum)
```
...
activity %>% 
        filter(weekdays(date) %in% days) %>%
        group_by(interval) %>% 
        summarise(meansteps = mean(steps, na.rm = TRUE)) %>%
        with(plot(interval, meansteps, main = "Weekday Average Activity",
                  xlab = "Time_of_Day_(hhmm)", 
                  ylab = "Steps per five minutes (ave)"))

# Calculate summary statistics weekday activity
activity %>% 
        filter(weekdays(date) %in% days) %>%
        group_by(date) %>% summarise(stepsum = sum(steps , na.rm = TRUE)) %>%
        summarise_each(funs(mean(.,na.rm = TRUE), 
